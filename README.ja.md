...
- **ファイル構造のみ**: 大きなメディア素材を含むプロジェクトでも、本文を読み込まずにツリーだけ出力できます。
...
- **「ファイル構造のみ」** を有効にすると、ファイル本文を読み込まず、`File Tree` だけをコピーしやすい形で出力します。
...
ルールには、ファイル名、フォルダ名、相対パス、簡単な `*` / `?` パターンを使えます。`.wav` のように拡張子だけを書いた場合も、その拡張子のファイルを本文出力から外します。フォルダ末尾の `/` は任意です。
